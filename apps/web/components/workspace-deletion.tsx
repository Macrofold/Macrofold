...
export function WorkspaceDeletion({ workspace }: { workspace: Schema['Workspace'] }) {
  ...
  return (
    <section className="panel danger-panel">
      <h2>{workspace.deletion_due_at ? 'Deletion scheduled' : 'Delete workspace permanently'}</h2>
      <p>
        {workspace.deletion_due_at
          ? `You can undo deletion before ${new Date(workspace.deletion_due_at).toLocaleString()}. New runs are paused.`
      ...
      {workspace.deletion_due_at ? (
        <Button
          ...
          onClick={async () => {
            setBusy(true);
            try {
              await api(`/v1/workspaces/${workspace.id}/deletion`, 'DELETE');
      ...
      >
        <form
          ...
          onSubmit={async (e) => {
            ...
            setError('');
            try {
              await api(`/v1/workspaces/${workspace.id}/deletion`, 'POST', {
                confirmation: name,
                ...(password ? { password } : {}),
              });
              setOpen(false);
        ...
        >
          ...
          <Button variant="danger" busy={busy} disabled={name !== workspace.name}>
