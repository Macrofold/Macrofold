...
vec3 transport(float time, out vec4 material, out float visible, out float cohesion) {
  ...
  if(u_mode>4.5 && seed>.88) layer=2.0;
  ...
  // remain in motion. Maximum definition is reached just before wind release.
  float definition=smoothstep(5.5,14.8,time);
  ...
  if(departureAge>0.0) {
    ...
    float progress=(stopPhase-.50)/.99;
    float materialSpeed=.999/.99*(progress/sqrt(progress*progress+.0004)
  ...
  // Fade across the first 70% of the readable grid, leaving its latter
  // columns fully exposed. Use the incoming plane, not camera depth.
  float inletFade=smoothstep(0.0,320.0,stream.x);
  visible=step(0.0,unwrapped)*mix(inletFade,1.0,smoothstep(.35,.90,guidance));
  ...
  visible*=mix(structuredBrightness,1.0,smoothstep(.60,.98,cohesion));
...
export function createRenderer(canvas, data) {
  ...
  try {
    ...
    const setVariation = (variation) => {
      ...
      if (!config) throw new Error('Unknown confluence variation.');
      gl.useProgram(program);
