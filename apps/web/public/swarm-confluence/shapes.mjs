...
float organicMember(float part, float mode) {
  float count = mode >= 2.5 ? 12.0 : mode >= 0.5 && mode < 1.5 ? 10.0 : 9.0;
...
// Twelve substantial pressure fronts share one slow downstream curvature.
// Their lanes fan apart with distance while depth bands retain the broad volume.
vec3 organicThalassa(float a, float angle, float layer, float member, float time) {
  ...
  float clock = time - 6.0;
  float phase = clock * 0.36 - lane * 0.29 + stratum * 0.17;
  float growth = 0.68 + 0.32 * smoothstep(2.0, 10.0, time);
  float fan = 0.25 + 0.75 * along;
  float cross = cap * cos(angle);
  float width = (0.29 + 0.16 * fan) * (1.0 + 0.16 * cos(2.1 * a - phase));
  float depth = 0.18 + 0.09 * fan + 0.035 * sin(2.7 * a + phase * 0.81);
  vec3 p = vec3(1.19 * a, cross * width * layer, cap * sin(angle) * depth * layer);
  p.y += lane * (0.31 + 0.035 * sin(clock * 0.31)) * fan;
  p.z += stratum * 0.30 * fan;
  // Long traveling crests slide across broad skins instead of shaking each lane.
  p.y += cap * (0.28 * sin(1.8 * a - clock * 0.34) + stratum * 0.075 * cos(2.0 * a + phase));
  p.z += cap * (0.25 * sin(2.7 * a + 1.25 * cross - phase)
    + 0.13 * cos(1.7 * a + clock * 0.29 + lane * 0.22));
  float front = max(0.0, sin(angle));
  float pocket = a - 0.28 * sin(phase * 0.79);
  p.z -= 0.17 * cap * front * front * exp(-4.0 * pocket * pocket - 2.4 * cross * cross);
  p.yz = rotate(p.yz, 0.28 * sin(1.55 * a - clock * 0.29) + stratum * 0.06);
  p.y *= growth * (0.97 + 0.085 * sin(clock * 0.33 + 1.9 * a));
  p.z *= growth * (0.98 + 0.09 * cos(clock * 0.27 - 1.6 * a));
  p.x += 0.055 * cap * sin(1.7 * cross - phase);
  return 0.96 * p / sqrt(vec3(1.0) + 0.14 * p * p);
}

// Three unequal rolls shear twelve thick fronts around adjacent moving cores.
// Counter-rotation and migrating crests reshape the torrent without a single pipe.
vec3 organicMaelstrom(float a, float angle, float layer, float member, float time) {
  ...
  float lane = mod(member, 4.0) - 1.5;
  float clock = time - 6.0;
  float growth = 0.66 + 0.34 * smoothstep(2.0, 10.0, time);
  float fan = 0.26 + 0.74 * along;
  float phase = clock * 0.92 + family * 1.87;
  float handedness = family < 0.5 ? 1.0 : family < 1.5 ? -1.0 : 0.83;
  float cross = cap * organicSignedPower(cos(angle), 0.87);
  float crest = 3.9 * a - phase;
  float width = (0.30 + 0.20 * fan) * (1.0 + 0.21 * sin(crest + lane * 0.39));
  float thickness = 0.20 + 0.095 * fan + 0.045 * cos(3.1 * a + phase * 0.79);
  vec3 p = vec3(1.17 * a, cross * width * layer, cap * sin(angle) * thickness * layer);
  p.y += lane * 0.20 * fan;
  p.z += cap * (0.25 * sin(crest + 1.7 * cross) + 0.14 * cos(2.5 * a - phase * 0.63 + lane * 0.31));
  p.y += cap * 0.14 * sin(2.4 * a + phase * 0.71 + 1.4 * cross);
  float roll = handedness * (phase + 3.2 * a + 0.66 * sin(2.5 * a - phase * 0.53));
  p.yz = rotate(p.yz, roll);
  float bearing = family * 2.13 + 0.28 * sin(clock * 0.47 + family);
  vec2 core = fan * vec2(0.36 * cos(bearing), 0.32 * sin(bearing));
  core += cap * vec2(0.15 * sin(2.1 * a + phase * 0.64), 0.13 * cos(2.3 * a - phase * 0.71));
  p.yz += core;
  p.yz = rotate(p.yz, 0.20 * sin(2.0 * a - clock * 0.43));
  p.y *= growth * (0.97 + 0.11 * sin(clock * 0.65 + 1.7 * a));
  p.z *= growth * (0.97 + 0.13 * cos(clock * 0.57 - 2.2 * a));
  p.x += cap * 0.10 * sin(2.0 * cross + family + phase * 0.59);
  return 0.97 * p / sqrt(vec3(1.0) + 0.15 * p * p);
}
...
vec3 polyDruseFrame(vec3 point, vec3 axis) {
  // The shortest rotation from +Y avoids a spinning tangent near the Z pole.
  // Druse's seven parent axes stay away from the opposite (-Y) direction.
...
vec3 polyhedralVolume(float axial, float angle, float layer, float part, float time, float mode) {
  ...
  if (mode < 1.5) {
    // Seven intergrown aggregates carry three terraces of four small crystals.
    // All eighty-four identities persist; changing overlap exposes different
    // facet layers without a trunk, a dominant block, or a timed birth gate.
    float aggregate = floor(member / 12.0);
    float terrace = floor(mod(member, 12.0) / 4.0);
    float crystal = mod(member, 4.0);
    ...
    // Successive terraces trade lateral reach and height. Their four crystals
    // share a slow turn but have independent support-plane proportions.
    float terracePhase = familyPhase + terrace * 1.17;
    float azimuth = crystal * PI * 0.5 + terrace * 0.49;
    azimuth += 0.31 * sin(phase * 0.58 + terracePhase);
    float radius = (0.205 - terrace * 0.032)
                    * (0.91 + 0.23 * sin(phase * 0.63 + terracePhase));
    float rise = (terrace - 1.0) * (0.12 + 0.045 * cos(phase * 0.52 + familyPhase));
    ...
    float inclination = 0.31 + terrace * 0.11 + 0.22 * sin(phase * 0.64 + local);
    vec3 axis = normalize(vec3(inclination * cos(azimuth), 1.0,
                                inclination * sin(azimuth)));
    float widthBase = terrace < 0.5 ? 0.086 : terrace < 1.5 ? 0.063 : 0.052;
    float heightBase = terrace < 0.5 ? 0.112 : terrace < 1.5 ? 0.185 : 0.153;
    ...
    return center + polyDruseFrame(point, familyAxis);
  }
