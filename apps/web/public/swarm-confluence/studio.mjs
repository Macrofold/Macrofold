...
status.textContent = 'Loading confluence study…';
let renderer;
let ready = false;
let disposed = false;
let visible = true;
...
function updateControls() {
  ...
  if (!poster.hidden) {
    ...
    poster.alt = original
      ...
      : surfacePoster
        ? `${posterName} confluence study at fourteen seconds.`
    ...
    (async () => {
      ...
      try {
        ...
        if (!response.ok || !response.headers.get('content-type')?.startsWith('image/webp'))
          throw new Error('The selected confluence poster is unavailable.');
        image = new Image();
