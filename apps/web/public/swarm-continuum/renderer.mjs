...
float entryOrder(float phase, float angle, float part) {
  ...
  float progress=smoothstep(.08,.88,phase+offset);
  return pow(progress,3.3);
}
...
vec3 transport(float time, out vec4 material, out float visible, out float cohesion) {
  ...
  bool hypercell=abs(u_mode-7.0)<.1;
  float stopAxial=.42+.53*fract(seed*53.17);
  float stopPhase=.12+.62*(stopAxial+1.0);
  if(hypercell) {
    stopPhase=.55+.85*(.80+.15*fract(seed*53.17));
    stopAxial=-1.0+2.0*smoothstep(.55,1.40,stopPhase);
  }
  float activePhase=min(phase,stopPhase);
  float axial=clamp((activePhase-.12)/.62-1.0,-.999,.999);
  // Cover both sides of the small projected cells after their particles have
  // gathered. A continuous material clock avoids leaving whole faces sparse.
  if(hypercell) axial=-.999+1.998*smoothstep(.55,1.40,activePhase);
  ...
  if(u_mode>4.5 && seed>(hypercell ? .56 : .80)) layer=2.0;
  ...
  if(departureAge>0.0) {
    ...
    float materialSpeed=1.0/.62;
    if(hypercell) {
      float progress=clamp((stopPhase-.55)/.85,0.0,1.0);
      materialSpeed=1.998*6.0*progress*(1.0-progress)/.85;
    }
...
void main() {
  ...
  if(abs(u_mode-7.0)<.1) {
    // Edge population supplies Hypercell's bright boundaries; the quieter
    // face interiors remain populated without obscuring overlapping cells.
    alpha*=mix(1.0,material.z>1.5 ? 1.55 : .58,cohesion);
  }
...
export function createRenderer(canvas, data) {
  ...
  try {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const row = cell % 240,
        column = Math.floor(cell / 240);
      samples.set([(column + 0.5) / 250, (row + 0.5) / 240, hash(i + 60001), hash(i + 120001)], i * 4);
    }
    upload(samples);
    ...
    const setVariation = (variation) => {
      ...
      if (!config) throw new Error('Unknown continuum variation.');
      gl.useProgram(program);
