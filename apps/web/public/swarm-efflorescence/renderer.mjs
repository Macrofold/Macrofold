...
float skinCoordinate(float phase) {
  // Open currents already have breadth upstream. Start their material travel
  // during the inlet transition so cohorts cannot pile up at a shared lip.
  bool current=u_mode>2.5 && u_mode<4.5;
  float p=(phase-(current ? .18 : .50))/(current ? 1.31 : .99);
...
vec3 transport(float time, out vec4 material, out float visible, out float cohesion) {
  ...
  vec3 center=u_mode>9.5 ? vec3(850.0,520.0,0.0) : vec3(1020.0,530.0,0.0);
  ...
  // remain in motion. Maximum definition is held for five seconds before wind release.
  ...
  float surfaceExponent=u_mode>2.5 && u_mode<9.5 ? 4.0 : 2.0;
  ...
  world+=drift*mix(13.0,.65,definition)*guidance;
  ...
  if(departureAge>0.0) {
    ...
    bool current=u_mode>2.5 && u_mode<4.5;
    float span=current ? 1.31 : .99;
    float progress=(stopPhase-(current ? .18 : .50))/span;
...
void main() {
  ...
  float releaseAt=16.0+.85*a_sample.y+.40*pow(sin(a_sample.y*17.0),2.0)+.30*seed;
  ...
  alpha*=1.0-smoothstep(19.3,20.0,time);
...
export function createRenderer(canvas, data) {
  ...
  try {
    ...
    const setVariation = (variation) => {
      ...
      if (!config) throw new Error('Unknown efflorescence variation.');
      gl.useProgram(program);
    ...
    return {
      ...
      render(seconds, pointerX = 0, pointerY = 0) {
        if (disposed) return;
        const time = Math.max(0, seconds % 21);
