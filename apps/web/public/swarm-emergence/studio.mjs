...
let surfacePoster = './surfaces/chimera.webp';
let posterName = 'Chimera';
let assetAbort;
const duration = 22;
const formationEnd = 15;
document.querySelector('#static-notice').hidden = true;
status.textContent = 'Loading emergence study…';
let renderer;
let ready = false;
let disposed = false;
let visible = true;
...
function updateControls() {
  ...
  if (!poster.hidden) {
    ...
    poster.alt = original
      ...
      : surfacePoster
        ? `${posterName} emergence study at nine seconds.`
    ...
    (async () => {
      ...
      try {
        ...
        if (!response.ok || !response.headers.get('content-type')?.startsWith('image/webp'))
          throw new Error('The selected emergence poster is unavailable.');
        image = new Image();
