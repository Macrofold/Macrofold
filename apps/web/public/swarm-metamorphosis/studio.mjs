...
let assetAbort;
const duration = 20;
const formationEnd = 7;
document.querySelector('#static-notice').hidden = true;
status.textContent = 'Loading metamorphosis study…';
let renderer;
let ready = false;
let disposed = false;
let visible = true;
...
function updateControls() {
  ...
  if (!poster.hidden) {
    ...
    poster.alt = original
      ...
      : surfacePoster
        ? `${posterName} metamorphosis study at nine seconds.`
    ...
    (async () => {
      ...
      try {
        ...
        if (!response.ok || !response.headers.get('content-type')?.startsWith('image/webp'))
          throw new Error('The selected metamorphosis poster is unavailable.');
        image = new Image();
