...
vec3 volume(float axial, float angle, float layer, float part, float time) {
  if(u_mode<4.5 || u_mode>9.5) return organicVolume(axial,angle,layer,part,time,u_mode);
  return polyhedralVolume(axial,angle,layer,part,time,u_mode-5.0);
}
float sectionOrder(float part, float time) {
  if(u_mode<4.5 || u_mode>9.5) return organicOrder(part,time,u_mode);
  return polyhedralOrder(part,time,u_mode-5.0);
}
float sectionPresence(float part, float time) {
  if(u_mode<4.5 || u_mode>9.5) return organicPresence(part,time,u_mode);
  return polyhedralPresence(part,time,u_mode-5.0);
}
float gathering(float time) {
  return smoothstep(.35,u_mode>9.5 ? 4.7 : 6.0,time);
}
float entryOrder(float phase, float angle, float part) {
  float offset=.06*sin(angle*2.0)+.035*cos(angle*3.0+part*2.0);
  // Currents and branching vessels acquire their surfaces farther upstream. The inlet still
  // crosses a broad transition rather than meeting a shared attachment seam.
  bool openStructure=(u_mode>2.5 && u_mode<4.5) || u_mode>9.5;
  float progress=smoothstep(openStructure ? .015 : .06,openStructure ? .60 : .76,phase+offset);
  return pow(progress,2.0);
}
// Nearly uniform material speed across a surface, with rounded tails. A
// smoothstep here would park too many particles near a closed surface pole.
float skinCoordinate(float phase) {
  // Open currents and connected vessels have breadth upstream. Start material travel
  // during the inlet transition so cohorts cannot pile up at a shared lip.
  bool openStructure=(u_mode>2.5 && u_mode<4.5) || u_mode>9.5;
  float p=(phase-(openStructure ? .18 : .50))/(openStructure ? 1.31 : .99);
  return .999*(sqrt(p*p+.0004)-sqrt((p-1.0)*(p-1.0)+.0004));
}
...
vec3 transport(float time, out vec4 material, out float visible, out float cohesion) {
  ...
  float cycle=1.85;
  float speed=.205;
  // Bring the first cohorts into the organizing region one second sooner,
  // then return smoothly to the established throughflow speed.
  float arrivalTime=time+1.0-exp(-time*1.3);
  float unwrapped=a_sample.x*cycle+arrivalTime*speed-cycle;
  float phase=mod(max(0.0,unwrapped),cycle);
  // First acquire shared motion; then travel over the complete skin. The
  // material clock spans both ends of every surface after the broad inlet
  // transition, instead of spending half the surface traversal still scattered.
  float stopPhase=1.39+.10*fract(seed*53.17);
  float stopAxial=skinCoordinate(stopPhase);
  float activePhase=min(phase,stopPhase);
  float axial=skinCoordinate(activePhase);
  float angle=2.0*PI*a_sample.y+time*(.13+.012*sin(seed*31.0));
  angle+=.032*sin(time*.72+axial*3.0+a_sample.y*6.0);
  float layer=seed<.06 ? .70+.28*fract(seed*37.0) : 1.0;
  if(u_mode>4.5 && u_mode<9.5 && seed>.88) layer=2.0;
  float part=a_sample.z;
  material=vec4(axial,angle,layer,part);
  bool current=u_mode>2.5 && u_mode<4.5;
  vec3 center=u_mode>9.5 ? vec3(850.0,520.0,0.0)
    : vec3(current ? 940.0 : 1020.0,530.0,0.0);
  // Longer Aperiodic spires use the open space beside the inlet; their new
  // camera angle and position preserve the extra length without zooming out.
  if(u_mode>8.5 && u_mode<9.5) center.x=820.0;
  if(u_mode>5.5 && u_mode<6.5) center=vec3(820.0,515.0,0.0);
  vec3 body=orient(volume(axial,angle,layer,part,time))*u_view.z+center;
  float lane=floor(a_sample.y*32.0+.5)/32.0-.5;
  // A shallow saddle fans out on the left, with curved columns and a turning
  // lower edge. Its depth projects into the rows instead of leaving a flat grid.
  float bend=phase*2.2+time*.055;
  float depth=160.0*sin(lane*2.6+bend*.65)+110.0*lane*lane;
  vec3 stream=vec3(-185.0+1850.0*phase+68.0*lane*lane+28.0*sin(lane*3.0+bend),
    565.0+lane*(610.0-165.0*sin(bend*.85))
      +62.0*sin(bend-.70)+120.0*lane*lane*sin(bend+.5),depth);
  stream.x+=depth*.20;
  stream.y-=depth*.27;
  // Disorder begins faintly among still-readable rows and builds across a
  // broad, curved region. No shared x coordinate switches all lanes at once.
  float spread=pow(smoothstep(.05,.86,phase+.08*sin(lane*4.2)+.04*sin(bend)),3.2);
  // Local order settles gradually while the material and large-scale geometry
  // remain in motion. Maximum definition lasts nine seconds before wind release.
  float definition=smoothstep(5.5,11.0,time);
  float regional=mix(sectionOrder(part,time),.997,definition*.90);
  ...
  float surfaceExponent=u_mode>2.5 ? 4.0 : 2.0;
  ...
  // Extra particles cover the faces themselves. Their normal-scale scatter
  // settles into clear skins rather than accumulating on highlighted edges.
  world+=drift*mix(u_mode>9.5 ? 6.0 : 13.0,.65,definition)*guidance;
  ...
  if(departureAge>0.0) {
    ...
    bool openStructure=(u_mode>2.5 && u_mode<4.5) || u_mode>9.5;
    float span=openStructure ? 1.31 : .99;
    float progress=(stopPhase-(openStructure ? .18 : .50))/span;
    float materialSpeed=.999/span*(progress/sqrt(progress*progress+.0004)
      -(progress-1.0)/sqrt((progress-1.0)*(progress-1.0)+.0004));
    vec3 tangent=(end-before)/.008*materialSpeed;
  ...
  // Double the inlet fade distance. Use the incoming plane, not camera
  // depth or gathering strength, so organization cannot bypass the fade.
  float inletFade=smoothstep(0.0,640.0,stream.x);
  visible=step(0.0,unwrapped)*inletFade;
  visible*=1.0-smoothstep(1760.0,2160.0,world.x);
  visible*=seed<.06 ? .45 : 1.0;
  visible*=mix(1.0,.10,departure);
  // Recycle only after a continuous fade, including trajectories whose rotated
  // surface endpoint has not yet crossed the right edge of the frame.
  visible*=1.0-smoothstep(cycle-.20,cycle,phase);
  float looseBrightness=mix(.70,mix(.26,.065,definition),gathering(time));
  // Preserve the readable inlet while gradually quieting the wandering
  // population that overlaps the mature surfaces farther downstream.
  float structuredBrightness=mix(.70,looseBrightness,smoothstep(.16,.58,spread));
  // As definition matures, keep the still-gathering population subtle. Bright
  // points describe the coherent skins instead of veiling their faces.
  visible*=mix(structuredBrightness,1.0,
    smoothstep(mix(.60,.88,definition),mix(.98,.995,definition),cohesion));
...
void main() {
  ...
  float releaseAt=20.0+.85*a_sample.y+.40*pow(sin(a_sample.y*17.0),2.0)+.30*seed;
...
export function createRenderer(canvas, data) {
  ...
  try {
    ...
    // Keep the original palette asset, with deterministic resampling for the
    // expanded population. Particle identities stay fixed at every quality level.
    const count = 80000;
    const appearance = new Float32Array(count * 12);
    for (let i = 0; i < count; i++) {
      const source = ((i * 19733) % 40000) * 12;
      appearance.set(data.subarray(source, source + 12), i * 12);
    }
    upload(appearance);
    ...
    for (let i = 0; i < count; i++) {
      const cell = (i * 19733) % count;
      const row = cell % 250,
        column = Math.floor(cell / 250);
      samples.set([(column + 0.5) / 320, (row + 0.5) / 250, hash(i + 60001), hash(i + 120001)], i * 4);
    }
    upload(samples);
    ...
    const setVariation = (variation) => {
      ...
      if (!config) throw new Error('Unknown myriad variation.');
      gl.useProgram(program);
    ...
    return {
      ...
      render(seconds, pointerX = 0, pointerY = 0) {
        if (disposed) return;
        const time = Math.max(0, seconds % 25);
