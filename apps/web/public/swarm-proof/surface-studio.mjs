...
const requestedId = new URL(location.href).searchParams.get('variation');
let variationIndex = Math.max(
  0,
  SURFACE_VARIATIONS.findIndex((item) => item.id === requestedId),
);
let surfacePoster = './surfaces/folded-sheet.webp';
let posterName = 'Folded sheet';
let assetAbort;
const duration = 22;
const formationEnd = 15;
document.querySelector('#static-notice').hidden = true;
status.textContent = 'Loading surface study…';
let renderer;
let ready = false;
let disposed = false;
let visible = true;
...
function updateControls() {
  ...
  if (!poster.hidden) {
    poster.src = original ? '/swarm-motion/swarm.webp' : surfacePoster || './current-v3-poster.webp';
    poster.alt = original
      ? 'The original Swarm artwork: a fine cyan and ivory particle structure on black.'
      : surfacePoster
        ? `${posterName} surface study at nine seconds.`
        : 'Previous Swarm flow study, not the selected surface variation.';
  }
  note.textContent = original
    ? 'Original reference'
    : !ready
      ? surfacePoster
        ? `${posterName} still · 9 s`
        : 'Previous flow still'
      : playback === 'formation'
...
function fail() {
  ...
  status.textContent = 'Live surface rendering is unavailable. A still is shown.';
  updateControls();
}
...
function chooseVariation(index) {
  ...
  if (renderer) {
    try {
      renderer.setVariation(SURFACE_VARIATIONS[variationIndex]);
      if (!playback) restoreStillQuality();
      status.textContent = `${SURFACE_VARIATIONS[variationIndex].name} ready.`;
    } catch {
      fail();
    }
    ...
    (async () => {
      ...
      try {
        ...
        if (!response.ok || !response.headers.get('content-type')?.startsWith('image/webp'))
          throw new Error('The selected surface poster is unavailable.');
        image = new Image();
...
select.replaceChildren(
  ...SURFACE_VARIATIONS.map((config, index) => {
    const option = document.createElement('option');
    option.value = config.id;
    option.textContent = `${String(index + 1).padStart(2, '0')} · ${config.name}`;
    return option;
  }),
);
...
listen(window, 'popstate', () => {
  ...
  chooseVariation(
    Math.max(
      0,
      SURFACE_VARIATIONS.findIndex((item) => item.id === id),
    ),
  );
});
updateVariationMetadata();
