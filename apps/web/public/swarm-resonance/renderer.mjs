...
vec3 transport(float time, out vec4 material, out float visible, out float cohesion) {
  ...
  world+=(field*112.0+drift*96.0)*pow(1.0-cohesion,surfaceExponent)*spread;
...
void main() {
  ...
  float alpha=visibility*a_shape.y*2.35*surfaceLight;
  ...
  float diameter=a_shape.x*u_scale*1.18;
...
export function createRenderer(canvas, data) {
  ...
  try {
    ...
    const setVariation = (variation) => {
      ...
      if (!config) throw new Error('Unknown resonance variation.');
      gl.useProgram(program);
