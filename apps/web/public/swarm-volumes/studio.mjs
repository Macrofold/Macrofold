...
let surfacePoster = './surfaces/cumulus.webp';
let posterName = 'Cumulus';
let assetAbort;
const duration = 22;
const formationEnd = 15;
document.querySelector('#static-notice').hidden = true;
status.textContent = 'Loading volume study…';
let renderer;
let ready = false;
let disposed = false;
let visible = true;
...
function updateControls() {
  ...
  if (!poster.hidden) {
    ...
    poster.alt = original
      ...
      : surfacePoster
        ? `${posterName} volume study at nine seconds.`
...
function fail() {
  ...
  status.textContent = 'Live volume rendering is unavailable. A still is shown.';
  updateControls();
}
    ...
    (async () => {
      ...
      try {
        ...
        if (!response.ok || !response.headers.get('content-type')?.startsWith('image/webp'))
          throw new Error('The selected volume poster is unavailable.');
        image = new Image();
