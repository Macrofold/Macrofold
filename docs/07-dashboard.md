...
The CLI is a client of these same sessions, workspaces and history. Explicit push/pull and Git review checkout are documented in [14-cli.md](14-cli.md). The dashboard is not a filesystem mount or a local synchronization daemon.
...
Errors identify missing configuration, authentication, ownership, discovery or grant problems. Test/discover actions inspect the protocol; invoking a metered provider tool remains subject to the execution budget and paid-execution guard. [19-tools-and-security.md](19-tools-and-security.md) describes the provider boundaries.
