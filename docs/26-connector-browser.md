...
The packaged standalone server then passed two additional browser smoke journeys covering catalog loading, desktop/mobile styling, local provider assets and pending app creation/removal. All 104 generated SDK routes match the current OpenAPI operations. The isolated review preview is available on `http://localhost:3211/connections` with the prefilled local demo sign-in.
