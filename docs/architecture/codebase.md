...
| [packages/providers](../../packages/providers/src)                   | Host allocation, object storage, Git and other provider adapters                                           | [Portability](../architecture/portability.md), [integrations](../features/identity-integrations/README.md) |
| [packages/runtime](../../packages/runtime)                           | Protected Host controller, supervisor and native harness adapters                                           | [Runtime implementation](../features/execution/runtime.md)                                                 |
...
## Workers and execution infrastructure

`packages/core/src/workers.ts` owns public CRUD, authorization, revision checks and batched observations. `worker-policy.ts` and `worker-types.ts` own resolved configuration/lifecycle policy; `worker-catalog.ts` validates deployment capabilities and accepted quotes. `worker-pricing.ts` owns exact cumulative money arithmetic. OpenAPI remains the public schema authority.

`host-allocations.ts` owns transactional resource claims, assignment history, cache hints, funding and meter cursors. `worker-placement.ts` and `worker-scaling.ts` calculate bounded choices; `worker-reconciler.ts` applies lifecycle intent and invokes providers outside domain transactions. `scheduling.ts` combines Worker feasibility with fair organization turns and Worktree ordering. `engine.ts` separates short admission from concurrent execution; local and cloud pollers retain their existing orchestration owners.

`packages/contracts/host-control.ts` defines the internal wire/port boundary. `packages/providers/src/hosts.ts` implements Docker, Vercel, Render and explicit simulator allocation adapters. `automatic-machines.ts` serves ordinary unpinned Runs; `worker-machines.ts` binds explicit Workers to HostRun assignments without adding another Run engine.

`packages/runtime/src/host-control.ts` owns run-indexed commands, authorized file materializations and bounded live harness handles. The supervisor/process helpers quiesce only the relevant writers; `host-meter.ts` records cumulative resource receipts. Native adapters own continuation IDs/files; the runtime is not a universal session filesystem or credential vault.

`packages/cli/src/workers.ts` and the existing CLI execution flags expose the same contract. Web Worker components and Run placement controls use the generated client. [Worker design](worker-execution.md), [current guide](../features/execution/workers.md), [verification](../features/execution/workers/verification.md), and [maintainer TODO](../maintainers/TODO.md) map behavior, evidence, and deferred checks. Historical migration/accounting records remain history rather than executable compatibility routes.
