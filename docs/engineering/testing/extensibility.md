...
- The final isolated optimized build passed **34 browser tests**, **five packaged CLI tests**, the real PTY journey, and Python HTTP/SSE persistence and session continuation. Browser coverage includes light/dark motion, reduced motion, forced colors, copy feedback, sidebar resizing, project layouts, persisted permission settings, file conflicts, drag and drop, and rich/source Markdown editing. The standalone worker started successfully with shared domain fingerprinting independent of the HTTP validator.
...
pnpm test:sdks
pnpm test:dashboard:isolated tests/browser/workspace-files.spec.ts tests/browser/dashboard-motion.spec.ts tests/browser/dashboard-interactions.spec.ts tests/browser/project-layout.spec.ts tests/browser/sidebar-layout.spec.ts tests/browser/copy-feedback.spec.ts --workers=1
pnpm check
pnpm docs:check
```
