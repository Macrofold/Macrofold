...
Starter suits a person running one foreground task and one independent task. Pro supports several repositories or a small team. Scale supports a power user doing a 50-worktree sweep or a substantial batch of independent workspaces. These prices are launch hypotheses, not validated willingness-to-pay or profitability guarantees. The existing Pro price and concurrency are preserved. Scale adds support/storage/control-plane room while billing every execution minute and model/tool call. Included credits are usage discounts, not unrestricted compute.

All plans default to a **24-hour queue deadline**, configurable downward per request. Execution defaults to 900 seconds, or a lower customer cap, and begins at provisioning admission. Provisioning, input waits, execution and persistence occupy slots; waiting in the queue does not. Account limits are maxima, not reservations. Interactive jobs get first consideration when slots open; no running job is interrupted. Unused capacity can be borrowed. See [scheduler and scaling design](../execution/scheduling.md).
...
Catalog token prices are the **retail** micro-USD per million rates. Cache reads use the full input rate; Anthropic cache writes use twice that rate. OpenAI cached input is already a subset and is not added again; reported output includes reasoning. This is a disclosed retail policy, not every vendor cache discount. Direct-provider retail prices are maintained in the built-in [model policy](../execution/models.md), while reviewed OpenRouter routes use validated discovery prices; there is no hidden automatic 20% markup. The offline estimator uses a separately stated assumed 20% markup. Keep those inputs consistent when choosing launch prices. BYOK never falls back to a managed key. BYOK does not remove platform compute/storage or separately managed connector charges. Simulation records zero model tokens and has no provider charge.
...
Admission locks the organization, reserves the run maximum and commits it with the run/outbox. Every model/tool call rechecks current actor, lease, deadline, cancellation, permitted route and remaining liability before upstream dispatch. The gateway conservatively bounds supported text/tool protocols and reviewed inline image inputs. [Media accounting](../media/implementation.md#native-images-and-money) owns image validation and token ceilings. Hosted billable tools, other native media and unsupported passthroughs remain rejected. Parallel requests cannot spend the same remaining allowance. An unsupported or unbounded route stays disabled.
...
Model and compute rate snapshots are frozen at run admission. A native run's budget must cover its full configured compute window before any funds are reserved; an insufficient budget returns `run_budget_too_small` with the required minimum. Model/tool calls share the remaining allowance. Existing runs created before compute snapshots were introduced continue using the operator's configured compute rate. Compute covers the configured execution window; provisioning, checkpointing, failures, retained emergency snapshots and control-plane activity remain operator costs. Existing reservations protect credit until settlement. Earliest-expiring eligible credit is consumed first, unused reservations are released, and unused expired inclusion is removed by an immutable expiration journal.
...
Top-up amounts are validated before creating a provider customer or order. A pending checkout can reuse its link; a completed or expired order requires a new idempotency key. Dispute notifications retrieve current Stripe state while holding the organization's accounting lock, including notifications created within the same second. A failed lookup rolls back both the receipt and financial writes so redelivery can retry safely. Stripe does not guarantee notification order; event creation timestamps are insufficient to reconcile dispute state. See [webhook ordering](https://docs.stripe.com/webhooks#event-ordering) and [dispute retrieval](https://docs.stripe.com/api/disputes/retrieve).
...
The model gateway owns run authorization, credential selection, reservations, settlement and uncertainty handling. `ModelProtocol` is the provider port for request preparation, wire headers and usage normalization; `packages/providers/src/model-protocols.ts` owns endpoint paths, native tool forms, output-limit fields and partial-frame handling. Normalized input tokens include cached reads and writes exactly once. Cache-write counts remain an explicit subset for the additional published charge. A new provider must supply a reviewed protocol adapter and billing policy; adding an endpoint alone does not authorize new hosted tool charges.

## Decisions and task allocations

Direct and bounded decisions use the existing gateway requests, wallet reservations and settlement journals. Frozen decision rate cards are separate from native harness capability policy. A task checks committed consumed budget plus outstanding child ceilings before child admission; it does not reserve money again. Missing usage remains provisional liability and terminal cancellation does not free it for another child. [Decision recovery](../decisions/implementation.md) explains late evidence and the deferred automatic correction path.

## Diagnostic tracing

The [tracing integration](../observability/README.md) exports committed token counts, reservation/budget/charge details and known provider-cost estimates. Langfuse cost totals represent customer charges, with BYOK model charges zero and provider estimates kept separately. It does not replace the ledger or alter settlement.

## Itemized usage API

`billing-usage.ts` owns `GET /v1/billing/usage`, an organization-authorized, paginated read of existing model/tool/storage facts and finalized consumption journals. The journal remainder after all model/tool charges is the compute entry; parent totals are not another billable entry. Required time bounds apply to each source's accrual/settlement timestamp, not run creation. Filters use parameterized SQL and current customer bindings; only selected financial fields leave the service. No Langfuse request or new accounting store is involved.

Migration 039 adds tenant/time and run lookup indexes only; apply it before release. It does not rewrite financial facts, requires no data backfill, and can remain during an application rollback. Customer labels are absent if their optional binding was removed. Provider estimates remain distinct from charges; missing usage remains null. The [public guide](usage.md) owns field/filter/time semantics and pagination limitations. [Billing API tests](../../../tests/integration/billing-usage.test.ts) exercise actual loopback HTTP and the typed SDK, exact charge reconciliation, date boundaries, pagination, filters and denied credentials using disposable PostgreSQL.
