...
The dashboard follows changes made through the API, CLI, other users, workers, and other tabs using one shared SSE subscription per browser tab. The subscription lives in the persistent shell, including the overview route. It carries refresh hints for run lifecycle, persisted worktree/checkpoint changes, and Git status. Detailed output and historical run replay continue through the existing per-run SSE system.
...
The client combines signals in a fixed one-second window and serializes refresh batches. Signals arriving during a fetch schedule another bounded batch. Existing cached data stays visible during refetches; navigation, scroll, selected worktree/file, and dirty editor drafts are not reset. Detailed run streams remain independently mounted only on run pages.
