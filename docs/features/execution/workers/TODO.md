...
- [ ] Measure standalone cloud-poller admission under contention. `portable-dispatch` advances up to 20 claims concurrently, while try-lock claims succeed only for the global fair turn; a cycle can admit nothing and wait for its short backoff. If measured latency matters, admit sequentially like local dispatch and keep execution parallel. See [reconciliation](reconciliation.md#full-tip-acceptance-repairs).
