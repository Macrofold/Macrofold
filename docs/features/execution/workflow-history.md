...
Failed claims persist `dispatch_jobs.available_at`. Retry ceilings rise from 5 to 10, 20, 40 and 60 seconds as submission age crosses 5, 15, 35 and 75 seconds. Stable per-run jitter uses 80–100% of that ceiling (48–60 seconds at steady state). The next timestamp never exceeds `queue_expires_at`. Deriving timing from the original submission prevents worker restarts from resetting backoff. Account/worktree-blocked jobs are filtered by SQL eligibility and need no individual timer or Workflow.

These are earliest eligible retry times, not promised wakeup times. Completion notifies dispatch and bypasses pending-job backoff, while retaining dispatch leases and rechecking authorization, worktree ordering, fair organization turns and account/global capacity. Other successful API mutations also notify dispatch. The one-minute Vercel Cron is the fallback; without a notification, a failed-claim backoff plus Cron alignment can take about two minutes, plus service delay. Jobs filtered solely by account/worktree eligibility can become candidates on the next sweep. Capacity is never promised while occupied.
...
Ready phases proceed immediately without a durable sleep. Active execution still polls at two seconds plus phase/provider latency; pending restore checks use three seconds. Cancellation and input answers follow that cadence during execution. Provider failures back off up to 60 seconds. A positive delay from the 128th advance is honored before handing off, so continuation does not shorten a requested wait. Handoff normally triggers immediate dispatch; a lost notification waits for Cron. An abandoned active Workflow lease is recoverable after five minutes plus Cron/dispatch delay; a caught start error schedules a 30-second lease. These are recovery targets, not an SLA. The native supervisor also enforces its original deadline independently.
...
A 60-second queue loop alone is not a safe solution. The existing maximum persistence envelope (100,000 entries, 10 GiB, four-MiB chunks) spans many invocations even with batched restoration. The deterministic budget test conservatively allows just one restored object per advance when the byte cap binds, two-hour execution, restore waiting and maximum transcript draining; it also adds two domain transport failures before every successful phase and three SDK retries per step. Bounded continuations keep each invocation within the same ceiling regardless of the total. Skipping ready-phase sleeps reduces actual history below the bound. This is a history bound, **not evidence that a 10-GiB transfer completes within sandbox lifetime or provider request quotas**.
...
Local acceptance covers these boundaries:

| Evidence | Scope |
| --- | --- |
| [History policy](../../../tests/unit/workflow-history.test.ts) | Full simulated 24-hour retry schedule, jitter/restart/expiry boundaries, production 128-advance ceiling, explicit waits, ready advances without sleeps, early exit, superseded owner and maximum-workload budget |
...
| [Cloud phase regression](../../../tests/integration/cloud.test.ts) and [scheduling cases](../../../tests/integration/scheduling.test.ts) | Batched hydration with lost upload acknowledgement, immediate SQL scheduling, phase timing persistence, lost native launch acknowledgement, competing pollers, failed-agent persistence, lost VM preservation and expiry before provider access |
| [Hydration](../../../tests/unit/execution-hydration.test.ts), [filesystem round trip](../../../tests/unit/manifest.test.ts), and [restore failures](../../../tests/unit/snapshot-failures.test.ts) | Object/byte/concurrency limits, hash/size verification, exact staged bytes, in-flight read draining, native session state, symlinks and atomic publication |
| [Vercel](../../../tests/unit/vercel-provider.test.ts) and [Docker](../../../tests/unit/docker-machines.test.ts) adapters | Existing compute port contracts and provider identity checks using protocol fixtures |

The nine domain/unit files above pass **101 tests** through `pnpm test:domain` with a disposable PostgreSQL 14 database and temporary encrypted storage. Native filesystem round trips run locally; Docker was unavailable and live Vercel execution was not exercised. TypeScript and documentation checks also pass. These checks establish control-plane and restore behavior, not deployed latency or paid-provider acceptance.

The startup-change load rerun passes all 11 scheduling and Workflow dispatch cases in 37.96 seconds with unchanged timeout gates and a disposable PostgreSQL 14 database. It verifies eight- and 50-slot workloads, later arrivals, worker recovery and dispatch handoffs. This duration is local test execution, not deployed run latency or a benchmark comparison. The [scheduling reference](scheduling.md#verification-and-measured-limits) owns its separate workload measurements. The fixture explicitly binds its unpaid synthetic runs to the fake Vercel provider; production provider-mismatch checks remain in force.
...
The conservative maximum-workload model accounts for execution/restore waits and trace draining as well as both input and output persistence. It deliberately treats every advance as a possible sleep for the history ceiling, even though ready phases no longer sleep. Strict TypeScript passes; the current startup checks do not establish a new aggregate coverage percentage or compiled cloud replay result. [Implementation status](../../status/README.md) records broader release evidence, and [startup acceptance](../../maintainers/TODO.md#startup-latency-acceptance) tracks remaining hosted measurement.
