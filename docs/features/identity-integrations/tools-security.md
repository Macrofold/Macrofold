...
Packages share the customer's worktree and agent UID. User-provided package environment credentials are therefore exposed to that sandbox process, as stated in the UI; these are not isolated secrets from the customer's own code. Infrastructure account credentials are never passed into the sandbox. Remote MCP OAuth is preferable when credentials should remain at the central broker.
