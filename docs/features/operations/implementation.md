...
The existing infrastructure capacity report and read-only management MCP also expose active executions versus the configured ceiling, eligible queued jobs, oldest eligible wait, submission-to-start mean/p95, and per-account wait summaries. These combine current capacity with the requested start-time window. See [scheduling metrics and scaling](../execution/scheduling.md) for field meanings, fairness limits, and the measured local load profile.
