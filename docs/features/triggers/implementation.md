...
Trigger dispatch calls the same run admission resolver with its saved workspace and preset. The session/run retains the preset ID and version, while the tool snapshot retains exact account binding and the accepted ceiling. Incoming payload fields are event data, never connection_access_overrides. Saved triggers expose no exception field. Current workspace/preset rules are evaluated anew for each admitted run; see [connector access implementation](../../engineering/testing/connection-access.md).
