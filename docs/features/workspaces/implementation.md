# Persistent worktrees, checkpoints and Git

Contributor reference. Start with the [feature guide](README.md) for user workflows.

## User model

A workspace owns independent worktrees. A worktree is a complete filesystem and Git clone with one active writer. A session belongs to a worktree and retains native harness state; a run is one bounded execution in that session. Parallel work starts in another worktree/branch from a verified checkpoint or Git ref. A local CLI link only records selection and does not upload files. Persistent workspaces are the default; explicitly ephemeral workspaces reject continuation that would imply retained working files.

Connect an existing repository using the GitHub App or explicit file transfers. Workspace instructions remain ordinary customer files and cannot change platform grants or funding policy.

## Worktree identity and branches

The public Worktree entity remains ID-addressed. Its `name` and `branch` response fields may be null. `worktree-names.ts` owns display-name uniqueness, name-to-branch derivation, and authorized saved-ref discovery; provider code lists Git refs without loading repository objects. `GET /v1/workspaces/{id}/worktree-options?name=…&branch=…` returns the saved branch choices and advisory validation. POST creation repeats validation under the workspace lock. Explicit existing branches restore their saved tree; new branches retain the selected checkpoint ancestry.

`nameWorktreeForRun` assigns an unnamed worktree a prompt-derived slug and collision suffix inside the first admission transaction, after validation and reservation. It makes no extra model call. Rejected admission rolls back naming. The same opaque ID, files, and sessions remain valid before and after assignment. The workspace lock precedes the worktree writer lock; concurrent admissions cannot publish two identities. Name-only creation derives a Git-safe branch; branch-only creation adopts that display name. Explicit new/existing modes enforce branch availability.
...
`GET /v1/worktrees/{id}/files` preserves flat recursive listing by default. With `recursive=false`, it derives and deduplicates direct child directories under `path` before applying filters, cursor, and limit. Directory entries contain `path`, `type: directory`, and the worktree revision; file size and hash are omitted. Listing remains on the published manifest during active execution.

`POST /v1/worktrees/{id}/folders` accepts `{path}` and creates its `.gitkeep` marker. `PATCH /v1/worktrees/{id}/file?path=source` accepts `{new_path}` and renames one file or symlink without following links or copying its content object. `PUT /v1/worktrees/{id}/file?path=target&create_only=true` atomically rejects an existing file or directory rather than overwriting it. `POST /v1/worktrees/{id}/files/duplicate` accepts `{path,new_path}` and reuses the source content object and mode without a browser download. These operations take the existing worktree writer lock, reauthorize the resource, check `If-Match`, reject path collisions, verify a checkpoint, and publish the worktree revision in one database transaction. Folder deletion and folder renaming are not provided by these operations.
...
Exports produce a standard Git bundle or `worktree/` plus a separate root `manifest.json` in a portable tar archive. User `manifest.json` cannot overwrite export metadata. Confined relative symlinks retain their identity; escaping links fail export. Public exports expose authorized workspace content, not vault keys or arbitrary session credentials. The internal database/object restore preserves native sessions; the customer archive is a workspace-file export, not a universal conversation migration format.

Diff pagination selects paths first. For each selected entry, combined checkpoint sizes must be below 512,000 bytes before either content object is read. Larger entries retain path/hash/change metadata and return `binary: null` without a patch. Small entries retain content verification and binary detection. This bounds content memory per selected diff entry; it does not make whole-worktree manifest indexing a streaming operation.

## Git implementation and synchronization

`isomorphic-git` maintains ordinary compatible objects and refs in trusted temporary directories; native agents use system Git in their own sandbox. Initial worktree creation, file saves, transfers and finalization create Git revisions where valid. `.gitignore` controls publication, not independent filesystem persistence. Safe metadata import excludes hooks, alternates, arbitrary config and metadata symlinks. Trusted maintenance never executes customer checkout filters or hooks.

Checkpoint maintenance reads and verifies imported Git metadata before use. If a resulting metadata file has identical bytes and belongs to the same organization, saving reuses its existing encrypted object instead of uploading history again. Changed metadata and the trusted replacement config are stored normally. This reduces object-store PUTs while preserving verification, history, and the storage maintenance fence; manifest and Git processing still scale with worktree size.
