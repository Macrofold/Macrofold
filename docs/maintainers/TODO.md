...
- [ ] Repair and complete the existing staging deployment: fresh CLI inspection confirms prior deployments, but the canonical health endpoint returns HTTP 503 with `status: unavailable`. Isolated database migrations, scoped storage access, verified sender and runtime-image publication now pass. Deploy the checked release and verify canonical health, default Stripe Portal, protected webhook reachability, destination activation, and Checkout-to-ledger acceptance. Reverify saved credentials and prices; do not recreate resources based on older setup notes or infer readiness from a successful build. Follow [project selection](../operations/launch-environment.md#staging-and-vercel-project-selection) and [billing activation](../operations/launch-integrations.md#activate-and-test-staging-billing).
...
- [ ] Complete real verification/reset email delivery. The staging sending domain is verified and its sending key is domain-scoped; delivery and production sender acceptance remain separate.
