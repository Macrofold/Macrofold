...
- [ ] Repair or explicitly relocate existing links to absent `output/` design/research artifacts before claiming a green repository documentation gate. The guidance adoption check found 156 such failures in untouched documents, identical under the original checker; see [evidence](../engineering/agent-guidance/verification.md#observed-adoption-checks). Preserve useful design evidence and do not add empty placeholder artifacts.
- [ ] Resolve the existing application/runtime TypeScript diagnostics before claiming a green `pnpm check`; the guidance checker/fixtures pass their strict focused check. See the same [validation record](../engineering/agent-guidance/verification.md#observed-adoption-checks); do not relax compiler settings to accept them.
