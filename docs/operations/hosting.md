...
The exact account, callback, secret and release commands are in [launch guide](launch-guide.md). [deployment reference](deployment.md) describes both container targets and scheduler portability. This document explains component ownership and operating decisions.
...
| Product/financial state | Neon PostgreSQL in a compatible application region (local fixture: 17) | Restricted login/pooling, backups/PITR, capacity |
...
For the implemented three-tier fair scheduler, migration 024, 24-hour queue deadlines, measured local load evidence and the step-by-step global ceiling procedure, use [execution scheduling and scaling](../features/execution/scheduling.md).
