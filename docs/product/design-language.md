...
Use the five pillars **Project → Worktrees → Checkpoints and Git Sync → API, CLI, UI → Connectors**:

- Type the incoming prompt letter by letter at the vertical center. Then shrink it into a left-hand input port as the Project context outline appears behind it; draw a dotted feed toward the worker.
