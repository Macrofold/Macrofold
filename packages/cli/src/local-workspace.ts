...
export type WorkspaceLink = z.infer<typeof linkSchema>;
...
export async function findLink(
  start = process.cwd(),
): Promise<{ root: string; link: WorkspaceLink } | undefined> {
  ...
  while (true) {
    ...
    try {
      ...
      if (parent.isSymbolicLink() || !parent.isDirectory())
        throw new Error('Refusing a symlinked workspace association.');
...
export async function writeLink(root: string, link: WorkspaceLink) {
...
export async function unlinkWorkspace(root: string) {
  await metadataDirectory(root);
  await rm(path.join(root, '.agent', 'link.json'), { force: true });
}
export async function loadBaseline(root: string, worktree: string): Promise<Baseline> {
  const directory = await metadataDirectory(root);
  const file = path.join(directory, `baseline-${z.uuid().parse(worktree)}.json`);
  try {
    const stat = await lstat(file);
...
export async function saveBaseline(root: string, worktree: string, baseline: Baseline) {
  const directory = await metadataDirectory(root);
  const file = path.join(directory, `baseline-${z.uuid().parse(worktree)}.json`),
