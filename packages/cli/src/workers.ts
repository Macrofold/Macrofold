...
export async function workerId(context: Context, selector: string): Promise<string> {
  ...
  do {
    ...
    for(const worker of page.data)if(worker.name===selector){
...
export async function workerRunOptions(context: Context): Promise<Pick<Schema['RunCreate'],'worker_id'|'memory_mib'|'cpu_millis'>> {
  const selector=stringOption(context.flags,'worker');
  const memory=context.flags['memory-mib'];const cpu=context.flags['cpu-millis'];
  if(!selector && (memory!==undefined || cpu!==undefined))throw new CliError('Per-Run resource overrides require --worker.');
  return {worker_id:selector?await workerId(context,selector):undefined,
    memory_mib:typeof memory==='number'?memory:undefined,cpu_millis:typeof cpu==='number'?cpu:undefined};
}
function input(flags: Options): Schema['WorkerCreate'] {
  ...
  for(const [flag,field] of [['min-instances','min_instances'],['max-instances','max_instances'],
    ['max-concurrency','max_concurrency'],['idle-timeout','idle_timeout_seconds']] as const)
...
export const workerCommands: Record<string,Handler>={
  ...
  'worker list':async({context,flags})=>({data:await(await context()).client.request('listWorkers',{
    params:{query:{limit:typeof flags.limit==='number'?flags.limit:25,cursor:stringOption(flags,'cursor')}}})}),
  'worker show':async({context,args})=>{
    const ctx=await context();return {data:await ctx.client.request('getWorker',{params:{path:{worker_id:await selected(ctx,args)}}})};
  },
  ...
  'worker update':async({context,args,flags})=>{
    const ctx=await context();const worker=await selected(ctx,args);
    const current=await ctx.client.request('getWorker',{params:{path:{worker_id:worker}}});
    return {data:await ctx.client.request('patchWorker',{params:{path:{worker_id:worker}},body:{...input(flags),
      name:stringOption(flags,'name'),expected_revision:typeof flags.revision==='number'?flags.revision:current.revision}})};
  },
  'worker pause':async({context,args,flags})=>{
    const ctx=await context();const worker=await selected(ctx,args);
    if(flags.force)await confirm(`Cancel active Runs and pause Worker ${terminalText(worker)}?`,Boolean(flags.yes));
    return {data:await ctx.client.request('pauseWorker',{params:{path:{worker_id:worker}},body:{force:!!flags.force}})};
  },
  'worker resume':async({context,args})=>{
    const ctx=await context();return {data:await ctx.client.request('resumeWorker',{params:{path:{worker_id:await selected(ctx,args)}}})};
  },
  'worker destroy':async({context,args,flags})=>{
    const ctx=await context();const worker=await selected(ctx,args);
    await confirm(`${flags.force?'Cancel active Runs and destroy':'Gracefully destroy'} Worker ${terminalText(worker)}? Durable files are preserved.`,Boolean(flags.yes));
    return {data:await ctx.client.request('destroyWorker',{params:{path:{worker_id:worker}},body:{force:!!flags.force}})};
  },
};
