...
export async function reconcileFinance(tx: Tx, org: string) {
  ...
  const reserved = (
    await tx.query(
      "SELECT (coalesce((SELECT sum(reservation_micro_usd) FROM runs WHERE status IN ('queued','provisioning','running','waiting_for_input','persisting')),0)+coalesce((SELECT sum(reserved_micro_usd) FROM hosts),0))::text AS value",
    )
  ).rows[0].value;
