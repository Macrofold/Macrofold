...
import { requireWorkspace, requireScopes } from './auth';
...
export function present<K extends Table>(row: Record<string, unknown>, table: K): Document<K> {
  ...
  return {
    ...
    ...(row.worktree_id ? { worktree_id: String(row.worktree_id) } : {}),
  };
}
export async function get<K extends Table>(
  ...
  if (p) {
    ...
    if (table === 'workspaces') requireWorkspace(p, doc.id);
    else if (doc.workspace_id) requireWorkspace(p, String(doc.workspace_id));
    else if (doc.worktree_id) {
      const worktree = await get(tx, 'worktrees', String(doc.worktree_id));
      requireWorkspace(p, String(worktree.workspace_id));
    }
  }
  return doc;
}
export async function list<K extends Table>(
  ...
  if (table === 'workspaces' || table === 'agents') {
    if (query.get('query')) {
  ...
  if (p.workspaceIds.length && table !== 'agents') {
    values.push(p.workspaceIds);
    conditions.push(
      table === 'workspaces'
        ? `id=ANY($${values.length}::uuid[])`
        : table === 'worktrees'
          ? `workspace_id=ANY($${values.length}::uuid[])`
          : `(data->>'workspace_id')=ANY($${values.length}::text[])`,
    );
  }
  values.push(limit + 1);
  const rows = (
...
export async function operation<K extends OperationKind>(
  ...
  if (authority.binding === 'worktree' && 'worktree_id' in result)
    workspaceId = (await get(tx, 'worktrees', result.worktree_id)).workspace_id;
  else if (authority.binding === 'checkpoint' && 'checkpoint_id' in result && result.checkpoint_id)
    workspaceId = (await get(tx, 'checkpoints', result.checkpoint_id)).workspace_id;
  else if (authority.binding === 'workspace' && 'workspace_id' in result)
    workspaceId = (await get(tx, 'workspaces', result.workspace_id)).id;
  assert(
    workspaceId || authority.binding === 'organization',
    500,
    'operation_authorization_missing',
    'The operation must be bound to its workspace.',
  );
  if (workspaceId) requireWorkspace(p, workspaceId);
