...
For model execution use the [AI workflow](../../.agents/skills/macrofold-ai/SKILL.md). A typed result proves shape, not truth or current authority. Reuse the caller's admission/accounting boundaries rather than introducing equivalent ones here. Preserve injectable transports, explicit receipts, byte/time limits and distinct failure/uncertainty outcomes.
