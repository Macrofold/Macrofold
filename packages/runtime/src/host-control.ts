...
type Handle = {
  id: string; uid: number; worktree: string; session: string | null; permission: string;
  checkpoint: string | null; sessionRevision: string; compatibility: string;
  runtime: LiveHarness; active: string | null; lastUsed: number; memoryMiB: number;
};
type Assignment = {
...
export class HostController {
  ...
  private async evict(handle: Handle) {
    if (handle.active) throw new Error('handle_in_use');
    await discardHarness(handle.runtime);
    const base = `/host-data/handles/${handle.id}`;
    await rm(base, { recursive: true, force: true });
    if (!this.writers.has(handle.worktree)) {
      const workspace = `/host-data/worktrees/${handle.worktree}`;
      await chown(workspace,0,0).catch(error => { if (error.code !== 'ENOENT') throw error; });
      await chmod(workspace,0o700).catch(error => { if (error.code !== 'ENOENT') throw error; });
    }
  ...
  private async trimCaches(requiredMiB = 0, keep?: Handle) {
    ...
    for (const [worktree,cache] of cached) {
      ...
      for (const handle of this.handles.values()) if (handle.worktree === worktree && !handle.active) await this.evict(handle);
      await rm(`/host-data/worktrees/${worktree}`, { recursive:true, force:true });
      this.files.delete(worktree); bytes -= cache.bytes; free += cache.bytes;
    }
  }
  ...
  private async allocate(request: Prepare): Promise<Assignment> {
    ...
    const fingerprint=createHash('sha256').update(JSON.stringify({
      run:request.run_id,worktree:request.worktree_id,session:request.session_id,view:request.permission_view,
      checkpoint:request.checkpoint_id,revision:request.session_revision,key:request.compatibility_key,resources:request.resources,
      prompt:parsed.prompt,harness:parsed.harness,model:parsed.model,deadline:parsed.deadline,
    })).digest('hex');
    const existing=this.assignments.get(request.assignment_id);
    if (existing) {
      if (existing.run!==request.run_id || existing.fingerprint!==fingerprint || existing.released) throw new Error('assignment_changed');
      return existing;
    }
    ...
    if (active.length>=config.concurrency || memory+request.resources.memory_mib>config.resources.memory_mib-headroom ||
      cpu+request.resources.cpu_millis>config.resources.cpu_millis) throw new Error('host_capacity');
    let handle=[...this.handles.values()].find(value=>!value.active && value.session!==null && value.session===request.session_id &&
      value.worktree===request.worktree_id && value.permission===request.permission_view &&
      value.checkpoint===request.checkpoint_id && value.sessionRevision===request.session_revision && value.compatibility===request.compatibility_key);
    if (handle) {
      const processes=await agentProcesses(handle.uid);
      if (!handle.runtime.child || handle.runtime.child.exitCode!==null || !processes.size ||
        [...processes.values()].some(state=>state!=='T') || (await agentMemoryMiB(handle.uid))>request.resources.memory_mib) {
        await this.evict(handle); handle=undefined;
      }
    }
    ...
    if (!filesReused && handle) { await this.evict(handle); handle=undefined; }
    if (!handle) {
      ...
      await appendFile('/etc/passwd',`agent${uid}:x:${uid}:${uid}:Macrofold execution:${home}:/usr/sbin/nologin\n`);
      handle={id:handleId,uid,worktree:request.worktree_id,session:request.session_id,permission:request.permission_view,
        checkpoint:request.checkpoint_id,sessionRevision:request.session_revision,compatibility:request.compatibility_key,
    ...
    const configuration: NativeConfiguration={...parsed,workspace:paths.workspace,stateHome:paths.home,hostRun:context,
  ...
  private command(value:Assignment, script:'restore'|'snapshot-page'|'stdio-call', args:string[]=[]): { child:ChildProcess; result:Promise<string> } {
    const child=spawn(process.execPath,[`/opt/platform/${script}.mjs`,...args],{
      env:{PATH:process.env.PATH,NODE_ENV:'production',PLATFORM_RUN_ID:value.run,PLATFORM_ASSIGNMENT_ID:value.id},
      stdio:['ignore','pipe','ignore'],
    });
    value.children.add(child);
    const result=new Promise<string>((resolve,reject)=>{
      let output='';
      child.stdout?.on('data',(bytes:Buffer)=>{output+=bytes.toString();if(Buffer.byteLength(output)>8*1024*1024)child.kill('SIGKILL');});
      child.once('error',reject);
      child.once('close',code=>{value.children.delete(child);code===0?resolve(output):reject(new Error('runtime_command_failed'));});
    });
    return {child,result};
  }
  async control(request: HostControlRequest): Promise<unknown> {
    ...
    if (request.action==='configure') return this.commands.run('allocation',async()=>{
      ...
      for (const directory of ['/host-data','/host-data/worktrees','/host-data/handles']) {
        await mkdir(directory,{recursive:true,mode:0o711}); await chmod(directory,0o711);
      }
      await this.health();
      return {};
    });
    ...
    if (value.released && !['probe','snapshot','chunk','release'].includes(request.action)) throw new Error('assignment_released');
    ...
    if (request.action==='answer') {
      await atomicJSON(`${value.directory}/answer.json`,{id:request.id,answer:request.answer}); return {};
    }
      ...
      switch(request.action) {
        ...
        case 'launch':
          ...
          value.supervision=supervise(`${value.directory}/config.json`,'/opt/platform/native-worker.mjs',value.handle.runtime)
            .catch(async()=>{
              ...
              await atomicJSON(`${value.directory}/result.json`,{output:'',outcome:'failure',failureCode:'supervisor_failed',
                persistence:'failed',completedAt:new Date().toISOString()});
            }).finally(()=>{value.finished=true;});
          return 'started';
        case 'snapshot': return JSON.parse(await this.command(value,'snapshot-page',[String(request.offset)]).result);
        case 'chunk': return {content:(await readFile(`${value.directory}/snapshot/chunks/${request.hash}`)).toString('base64')};
        case 'stdio':
        ...
        case 'release': return this.commands.run('allocation', async () => {
          ...
          if(clean) {
            ...
            value.handle.memoryMiB=await agentMemoryMiB(value.handle.uid);
          }
          if(!clean || !value.handle.runtime.child || value.handle.runtime.child.exitCode!==null)await this.evict(value.handle);
          if(this.metered)await this.meter.release(value.id);
          value.released=true;
...
async function main() {
  if(process.platform!=='linux'||process.getuid?.()!==0)throw new Error('Isolated Linux root required');
  ...
  const server=createServer(async(req,res)=>{
    ...
    try{
      ...
      for await(const chunk of req){const bytes=Buffer.from(chunk);size+=bytes.length;if(size>8*1024*1024)throw new Error('request_too_large');chunks.push(bytes);}
      const envelope=z.object({boot_id:z.uuid().optional(),request:hostControlRequest}).parse(JSON.parse(Buffer.concat(chunks).toString()));
      if(envelope.request.action!=='health'&&envelope.boot_id!==controller.boot)throw new Error('host_restarted');
