...
function distribution(values: number[]) {
  ...
  const percentile = (fraction: number) => Math.round((sorted[Math.min(sorted.length - 1, Math.ceil(sorted.length * fraction) - 1)] || 0) * 100) / 100;
  return { samples: sorted.length, p50_ms: percentile(0.5), p95_ms: percentile(0.95), p99_ms: percentile(0.99), max_ms: percentile(1) };
}
async function parallel<T>(count: number, concurrency: number, action: (index: number) => Promise<T>): Promise<T[]> {
  const output: T[] = new Array(count);
  let next = 0;
  await Promise.all(Array.from({ length: Math.min(count, concurrency) }, async () => {
    while (next < count) { const index = next++; output[index] = await action(index); }
  }));
  return output;
}
...
async function workload() {
  ...
  if (config.mode !== 'local' || config.execution !== 'simulator' || config.allowPaid || !process.env.DATABASE_URL?.includes('platform_test_'))
  ...
  const stats = { accepted_runs: 0, completed_runs: 0, scheduling_deferrals: 0, peak_execution_concurrency: 0,
    peak_hosts: 0, observed_allocations: 0, rounds: 0, violations: 0, paid_api_calls: 0 };
  ...
  const server = createServer(async (incoming, outgoing) => {
    try {
      ...
      const response = await handleApi(new Request(config.origin + incoming.url, {
        method: incoming.method, headers,
        ...(bytes ? { body: Buffer.concat(chunks).toString('utf8') } : {}),
      }));
      outgoing.writeHead(response.status, Object.fromEntries(response.headers));
      outgoing.end(Buffer.from(await response.arrayBuffer()));
    } catch { outgoing.writeHead(500, { 'content-type': 'application/json' }); outgoing.end('{"error":{"code":"workload_transport_failed"}}'); }
  });
  try {
    ...
    const client = new Client({ baseURL: `http://127.0.0.1:${address.port}`, apiKey: account.key, retries: 0 });
    const worker = await client.request('createWorker', { body: {
      name: 'Bounded agent load', compute: 'server', dedicated: true, isolate_runs: false,
      min_instances: 0, max_instances: 4, max_concurrency: 32, max_hourly_compute_cost_micro_usd: '1000000',
    }});
    await parallel(20, 4, async index => client.request('createWorker', { body: {
      name: `Dormant observation ${index}`, compute: 'server', dedicated: true, isolate_runs: false,
      max_instances: 1, min_instances: 0, max_hourly_compute_cost_micro_usd: '1000000',
    }}));
    lag.enable();
    const accepted = await parallel(128, 8, async index => {
      const workspace = await client.request('createWorkspace', { body: { name: `Character ${index}` } });
      const at = performance.now();
      const result = await client.request('createRun', { body: {
        worktree_id: workspace.default_worktree_id!, worker_id: worker.id, prompt: 'Perform one bounded synthetic character turn.',
        harness: 'codex', model: 'fixture-model', billing_mode: 'managed', memory_mib: 256, cpu_millis: 125,
        queue_timeout_seconds: 600, limits: { timeout_seconds: 30, max_cost_micro_usd: '100000' },
      }});
      timings.admission.push(performance.now() - at);
      if (result.worker_id !== worker.id) throw new Error('Worker identity was lost at the public API boundary.');
      stats.accepted_runs++;
      return result;
    });
    const syntheticExecution: ExecutionProvider = {
      async execute(input, emit) {
        ...
        try {
          ...
          }], resumeState: '{"turn":1}', inputTokens: 0, outputTokens: 0, usageComplete: true };
        } finally { activeExecutions--; timings.execution.push(performance.now() - at); }
      },
    };
    const remaining = new Set(accepted.map(run => run.run_id));
    const deadline = Date.now() + 240000;
    async function observe() {
      const hosts = await transaction(org, tx => tx.query<{ occupied: number; memory: number; cpu: number; capacity: number; memory_mib: number; cpu_millis: number }>(
        `SELECT h.capacity,h.memory_mib,h.cpu_millis,count(a.id)::integer AS occupied,
         ...
         WHERE h.worker_id=$1 AND h.status<>'stopped' GROUP BY h.id`, [worker.id]));
      ...
      for (const host of hosts.rows) {
        if (host.occupied > host.capacity || host.cpu > host.cpu_millis || host.memory > host.memory_mib - Math.min(512, Math.ceil(host.memory_mib / 8))) {
          stats.violations++; throw new Error('Host resource ownership exceeded its accepted allocation.');
        }
      }
    }
    ...
    await client.request('listBillingUsage', { params: { query: { worker_id: worker.id, from: new Date(Date.now() - 3600000).toISOString(), to: new Date().toISOString() } } });
    ...
    if (paused.status !== 'paused' || paused.occupied_slots !== 0) throw new Error('The completed workload retained active compute claims.');
    ...
    const result = { status, failure, source_commit: execFileSync('git', ['rev-parse', 'HEAD'], { encoding: 'utf8' }).trim(), node: process.version,
      ...
      event_loop_p99_ms: Number.isFinite(lag.percentile(99)) ? Math.round(lag.percentile(99) / 10000) / 100 : null,
...
if (process.argv.includes('--fixture')) await workload();
  ...
  await withFixtureDatabase(async env => {
    const child = spawn('pnpm', ['exec', 'tsx', 'scripts/workers/stress.ts', '--fixture'], {
      env: { ...env, API_RATE_LIMIT_PER_MINUTE: '100000' }, stdio: 'inherit', timeout: 300000,
    });
