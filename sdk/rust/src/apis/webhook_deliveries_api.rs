...
/// struct for typed errors of method [`replay_webhook_delivery`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
pub async fn list_webhook_deliveries(configuration: &configuration::Configuration, cursor: Option<&str>, limit: Option<i32>, x_organization_id: Option<&str>) -> Result<models::ListWebhookDeliveries200Response, Error<ListWebhookDeliveriesError>> {
    ...
    let uri_str = format!("{}/v1/webhook-deliveries", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ListWebhookDeliveries200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ListWebhookDeliveries200Response`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn replay_webhook_delivery(configuration: &configuration::Configuration, delivery_id: &str, idempotency_key: &str, body: serde_json::Value, x_organization_id: Option<&str>) -> Result<models::Operation, Error<ReplayWebhookDeliveryError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ReplayWebhookDeliveryError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
