...
impl Client { pub fn tasks(&self) -> TasksResource<'_> {TasksResource {client:self, options:RequestOptions::default()}}
...
pub async fn replay(&self, delivery_id: &str) -> Result<models::Operation,ClientError> {
        ...
        let key = self.options.idempotency_key.clone().unwrap_or_else(||uuid::Uuid::new_v4().to_string());
        crate::apis::webhook_deliveries_api::replay_webhook_delivery(self.client.configuration(), delivery_id, &key, std::collections::HashMap::new(), self.options.organization.as_deref()).await
          .map_err(|error|crate::request_error(error,Some(key)))
      }
    }
...
pub struct RunsResource<'a> {client:&'a Client,options:RequestOptions}
    impl<'a> RunsResource<'a> {
      ...
      pub async fn cancel(&self, run_id: &str) -> Result<models::Run,ClientError> {
        ...
        let key = self.options.idempotency_key.clone().unwrap_or_else(||uuid::Uuid::new_v4().to_string());
        crate::apis::runs_api::cancel_run(self.client.configuration(), run_id, &key, serde_json::json!({}), self.options.organization.as_deref()).await
          .map_err(|error|crate::request_error(error,Some(key)))
      }
...
pub async fn list_events(&self, run_id: &str, params: ListRunEventsParams) -> Result<models::ListCustomerAgentRunEvents200Response,ClientError> {
...
pub async fn delete(&self, trigger_id: &str) -> Result<models::DeleteSlackConnection200Response,ClientError> {
...
pub async fn list_run_events(&self, customer_id: &str, customer_agent_id: &str, run_id: &str, params: ListCustomerAgentRunEventsParams) -> Result<models::ListCustomerAgentRunEvents200Response,ClientError> {
...
pub async fn delete(&self, connection_id: &str) -> Result<models::DeleteSlackConnection200Response,ClientError> {
...
pub async fn create(&self, input: models::InferenceCreate, params: CreateInferenceParams) -> Result<models::InferenceResponse,ClientError> {
        if input.stream == Some(true) { return Err("use inferences().stream for incremental output".into()); }
        let key = self.options.idempotency_key.clone().unwrap_or_else(||uuid::Uuid::new_v4().to_string());
