  ...
  try {
    ...
    for (const [kind, label, tool] of [
      ...
      if (kind === 'Web search') {
        ...
        await page
          .getByLabel('Brave Search API key', { exact: true })
          .fill('fixture-not-a-live-provider-key');
    ...
    // These connections are organization-wide in the shared fixture account; later journeys
    // (such as template schedule review) must not inherit them as eligible tools.
    const listed = await page.request.get('/v1/connections?limit=100', {
      headers: { Origin: fixtureOrigin },
    });
    const connections = ((await listed.json()).data || []) as { id: string; name: string }[];
    for (const connection of connections.filter((c) => created.includes(c.name))) {
      const deleted = await page.request.delete(`/v1/connections/${connection.id}`, {
        headers: { Origin: fixtureOrigin, 'Idempotency-Key': randomUUID() },
      });
      expect.soft(deleted.ok(), await deleted.text()).toBe(true);
    }
  }
});
