...
test('every navigation icon animates once on hover, replays on reentry, and keeps its link geometry', async ({
  ...
  const projects = navigationLink(page, 'Projects');
  await projects.scrollIntoViewIfNeeded();
  const before = await projects.boundingBox();
  await projects.hover();
  expect(await activeIconAnimations(projects)).toContainEqual({ name: 'sidebar-icon-folder', iterations: 1 });
  await projects.locator(':scope > svg').evaluate(async (icon) => {
    await Promise.all(icon.getAnimations({ subtree: true }).map((animation) => animation.finished));
  });
  expect(await activeIconAnimations(projects)).toEqual([]);
  expect(await projects.boundingBox()).toEqual(before);
...
test('collapsed icons animate on keyboard focus while their tooltip and navigation remain usable', async ({
  ...
  await expect(page.locator('#workspace-sidebar')).toHaveCSS('width', '64px');
});
...
test('dashboard playback overrides Reduce Motion and a paused choice persists', async ({ page }) => {
  ...
  const projects = navigationLink(page, 'Projects');
  await projects.hover();
  expect((await activeIconAnimations(projects)).length).toBeGreaterThan(0);
