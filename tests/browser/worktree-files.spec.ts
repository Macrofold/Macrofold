...
async function workspace(page: Page) {
  ...
  return { ...value, root: `/v1/worktrees/${value.default_worktree_id}` };
}
...
test('workspace and worktree permission editors persist independent restrictions', async ({ page }) => {
  const value = await workspace(page);
  await page.getByRole('tab', { name: 'Settings', exact: true }).click();
  const workspaceForm = page
    .locator('form')
    .filter({ has: page.getByRole('heading', { name: 'Workspace permissions', exact: true }) });
  const worktreeForm = page
    .locator('form')
    .filter({ has: page.getByRole('heading', { name: 'Worktree permissions', exact: true }) });
  const excluded = workspaceForm.getByRole('textbox', { name: 'read excluded files', exact: true });
  ...
  await workspaceForm.getByRole('button', { name: 'Save workspace permissions', exact: true }).click();
  await expect
    .poll(async () => (await (await page.request.get(`/v1/workspaces/${value.id}`)).json()).permissions)
...
test('rename waits for authoritative destination content after a concurrent source edit', async ({
  ...
  try {
    const worktree = await (await page.request.get(value.root)).json();
...
test('file viewer grows with the viewport and explorer resizing persists without overflow', async ({
  ...
  await page.screenshot({ path: 'test-results/file-worktree-desktop.png' });
  ...
  await expect
    .poll(async () => {
      const sidebar = (await page.locator('#worktree-sidebar').boundingBox())!;
      return Math.round(sidebar.x + sidebar.width);
    })
    .toBeLessThanOrEqual(0);
  await page.screenshot({ path: 'test-results/file-worktree-mobile.png' });
});
...
test('worktree dialog derives names and branches, rejects duplicates and supports deferred names', async ({
  ...
  const created = (await (await page.request.get(`/v1/workspaces/${value.id}/worktrees`)).json()).data.find(
  ...
  expect(await (await page.request.get(`/v1/worktrees/${id}`)).json()).toMatchObject({
    name: null,
    branch: null,
  });
