...
export async function writeFixtureFile(p: Principal, worktreeId: string, path: string, bytes: Buffer, revision: string) {
  const prepared = await prepareFileMutation(p, worktreeId, revision, { kind: 'file_write', path, bytes, createOnly: false });
  try {
