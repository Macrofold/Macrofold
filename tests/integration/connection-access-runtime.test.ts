...
async function setup() {
  ...
  const state = await transaction(a.p.organizationId, async (tx) => {
    const workspace = await r.create(tx, 'workspaces', a.p.organizationId, { name: 'Policy workspace' });
    ...
    return { workspace, agent, connection };
  });
...
it('allows replacement rules without expanding the accepted maximum and keeps preset origin through deletion', async () => {
  const f = await setup();
  const first = await f.tx((tx) => access.saveRule(tx, f.a.p, f.connection.id, { scope: 'workspace_agent', workspace_id: f.workspace.id, agent_id: f.agent.id }, '"2"'));
  const accepted = await f.tx((tx) => admitRun(tx, f.a.p, { workspace_id: f.workspace.id, agent_id: f.agent.id, prompt: 'Freeze access' }));
  ...
  const replacement = await f.tx((tx) => access.saveRule(tx, f.a.p, f.connection.id, { scope: 'workspace', workspace_id: f.workspace.id }, '"3"'));
...
it('rechecks the issuing API key write scope and current owner membership for an exception', async () => {
  ...
  const accepted = await f.tx((tx) => admitRun(tx, principal, { workspace_id: f.workspace.id, harness: 'codex', model: 'fixture-model', billing_mode: 'managed', prompt: 'Scoped exception', connection_access_overrides: [{ connection_id: f.connection.id, tools: ['web_search'] }] }));
  const currentTools = () => f.tx(async (tx) => runtimeConnectionTools(tx, await getRun(tx, accepted.run_id), await r.get(tx, 'connections', f.connection.id)));
  expect(await currentTools()).toEqual(['web_search']);
...
it('resolves beyond the preview page and rejects an oversized automatic set without truncation', async () => {
  const f = await setup();
  const context = { workspace_id: f.workspace.id, agent_id: null, permissions: [] };
