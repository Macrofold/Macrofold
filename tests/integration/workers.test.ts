...
import type { HostProvider, HostBinding, HostControlRequest } from '../../packages/contracts/host-control';
import { WorkerMachines } from '../../packages/core/src/worker-machines';
...
describe('fenced Host assignments through the Worker machine port',()=>{
  it('sends only the claimed assignment identity and refuses stale bindings, generations and releases without Host I/O',async()=>{
    const created=await worker({min_instances:1,max_instances:1});
    await ensureBaseline(created.id);
    const run=await newRun(created.id);
    const assignment=await tx(t=>claimHostRun(t,run));
    ...
    const recording:HostProvider={...provider,async control(binding,secret,request){
      requests.push(request);
      if(request.action==='prepare')return {reused:false,restoreNamespaces:['home']};
      if(request.action==='stage'||request.action==='release')return {};
      return provider.control(binding,secret,request);
    }};
    const machines=new WorkerMachines(run,()=>recording);
    const binding=await machines.provision();
    expect(binding).toEqual({name:`run-${run.id}`,sessionId:assignment.id,createdAt:assignment.claimed_at.toISOString()});
    const configuration:NativeConfiguration={runId:run.id,harness:'codex',model:'fixture-model',provider:'openai',prompt:'fixture',
      workspace:'/workspace',stateHome:'/agent-home',gatewayURL:'http://127.0.0.1:9/',toolURL:'http://127.0.0.1:9/',token:'fixture-token',
      deadline:new Date(Date.now()+60000).toISOString(),toolGrants:false};

    await expect(machines.prepare(binding,{...configuration,runId:id()})).rejects.toMatchObject({status:400,code:'invalid_configuration'});
    await expect(machines.stage(binding,[{path:'/platform-control/config.json',content:Buffer.from('{}')}]))
      .rejects.toMatchObject({status:400,code:'invalid_stage_path'});
    expect(requests).toEqual([]);

    expect(await machines.prepare(binding,configuration)).toEqual({reused:false,restoreNamespaces:['home']});
    expect(requests[0]).toMatchObject({action:'prepare',run_id:run.id,assignment_id:assignment.id,worktree_id:run.worktree_id,
      session_id:run.session_id,resources:{memory_mib:assignment.memory_mib,cpu_millis:assignment.cpu_millis}});
    expect((await tx(t=>t.query('SELECT state FROM host_runs WHERE id=$1',[assignment.id]))).rows[0].state).toBe('prepared');
    await machines.stage(binding,[{path:`/platform-control/restore/page-0.json`,content:Buffer.from('[]')}]);
    expect(requests[1]).toMatchObject({action:'stage',assignment_id:assignment.id,files:[{path:'page-0.json',content:Buffer.from('[]').toString('base64')}]});

    const before=requests.length;
    await expect(machines.stage({...binding,createdAt:new Date(0).toISOString()},[]))
      .rejects.toMatchObject({status:409,code:'host_assignment_changed'});
    await tx(t=>t.query('UPDATE hosts SET generation=generation+1 WHERE id=$1',[assignment.host_id]));
    await expect(machines.stage(binding,[])).rejects.toMatchObject({status:409,code:'host_generation_changed'});
    await tx(t=>t.query('UPDATE hosts SET generation=generation-1 WHERE id=$1',[assignment.host_id]));
    expect(requests).toHaveLength(before);

    expect(await machines.close(binding,false)).toEqual({});
    expect(requests.at(-1)).toMatchObject({action:'release',assignment_id:assignment.id,persisted:false});
    expect((await tx(t=>activeHostRun(t,run.id)))).toBeFalsy();
    const released=requests.length;
    await expect(machines.stage(binding,[])).rejects.toMatchObject({status:409,code:'host_assignment_lost'});
    // Repeated cleanup is idempotent and does not contact the Host again.
    expect(await machines.close(binding,true)).toEqual({snapshotId:assignment.id});
    expect(requests).toHaveLength(released);
  });
});
