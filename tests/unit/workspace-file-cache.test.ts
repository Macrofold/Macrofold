...
const workspaceId = '01994000-0000-7000-8000-000000000001';
const prefix = `/v1/workspaces/${workspaceId}`;
const timestamp = '2026-09-10T00:00:00.000Z';
const workspace: Schema['Workspace'] = {
  id: workspaceId,
  project_id: '01994000-0000-7000-8000-000000000002',
...
describe('confirmed workspace file cache mutations', () => {
  let client: QueryClient;
  beforeEach(() => {
    client = new QueryClient({ defaultOptions: { queries: { retry: false, staleTime: Infinity } } });
    client.setQueryData([prefix], workspace);
    client.setQueryData(directoryKey(), listing([file('kept.txt')]));
  });
  afterEach(() => client.clear());

  it('seeds the committed workspace revision and entry while retaining existing data', async () => {
    const confirmed = operation();
    expect(await publishFileMutation(client, workspaceId, confirmed)).toEqual(confirmed.result);
  ...
  it('moves a renamed file between direct folder caches without duplicating it across pages or replay', async () => {
    ...
    await publishFileMutation(client, workspaceId, confirmed);
    await publishFileMutation(client, workspaceId, confirmed);
  ...
  it('does not mutate or invalidate unrelated workspace and account queries', async () => {
    const otherWorkspace = '/v1/workspaces/01994000-0000-7000-8000-000000000099';
    const keys = [
      [otherWorkspace],
      [`${otherWorkspace}/files?path=&recursive=false&limit=100`],
      ['/v1/identity'],
      ['/v1/billing'],
      ['/v1/runs?limit=100'],
    ];
    ...
    await publishFileMutation(client, workspaceId, operation());
  ...
  it.each([
    ...
    const workspaceState = client.getQueryState([prefix]);
    const listingState = client.getQueryState(directoryKey());
    await expect(publishFileMutation(client, workspaceId, unconfirmed)).rejects.toThrow(
      'Unable to confirm the saved revision',
    );
    expect(client.getQueryState([prefix])).toBe(workspaceState);
