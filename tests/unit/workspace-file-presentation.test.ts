...
describe('workspace Markdown navigation', () => {
  ...
  it('gives duplicate formatted headings distinct prefixed anchors', () => {
    ...
    workspaceHeadingAnchors()({ type: 'root', children: [first, second] });
    expect(first.properties).toMatchObject({
      id: 'workspace-heading-install-now',
      'data-workspace-heading': 'install-now',
    });
    expect(second.properties).toMatchObject({ id: 'workspace-heading-install-now-1' });
  });
});
